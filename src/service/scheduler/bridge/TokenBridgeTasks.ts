...
/**
 * EthNet 의 토큰과 와 BizNet 의 토큰을 서로 교환하는 작업을 진행하는 스케줄러 클래스
 */
export class TokenBridgeTasks extends BridgeTasks {
    public _contract_manager: TokenBridgeContractManager | undefined;

    /**
     * 스마트컨트랙트의 인스턴스를 제공해 주는 인스턴스
     * @private
     */
    private get contract_manager(): TokenBridgeContractManager {
    ...
    public setOption(options: any) {
        if (options) {
            ...
            if (
                options.token_bridge_contract_manager &&
                options.token_bridge_contract_manager instanceof TokenBridgeContractManager
            )
                this._contract_manager = options.token_bridge_contract_manager;
        }
    }
    ...
    public async checkDepositIsOpened(swap: IBridgeSwapInfoInternal) {
        ...
        const task = async () => {
            try {
                return { deposit_box: await ContractUtils.getTokenBridgeDepositLockBoxInfo(source_bridge, swap.id) };
            } catch (error) {
    ...
    public async openWithdraw(swap: IBridgeSwapInfoInternal) {
        // Deposit Box 의 정보와 데이타베이스의 SWAP 정보가 동일한지 한번더 검증한다 (데이타베이스 검증)
        {
            ...
            const task_deposit = async () => {
                try {
                    return {
                        deposit_box: await ContractUtils.getTokenBridgeDepositLockBoxInfo(source_bridge, swap.id),
                    };
                } catch (error) {
                    const vm_error = ContractUtils.getVMError(error);
                    logger.error("Failed to check deposit lock box opening", {
        ...
        const task = async () => {
            try {
                return { withdraw_box: await ContractUtils.getTokenBridgeWithdrawLockBoxInfo(target_bridge, swap.id) };
            } catch (error) {
                const vm_error = ContractUtils.getVMError(error);
        ...
        if (withdraw_box.state === BridgeLockBoxStates.INVALID) {
            const token = this.contract_manager.getTargetTokenBySource(swap.direction, swap.deposit_token_id);
            if (token === null) {
                logger.error("Can not found token info. corresponding to the token ID of the deposit box.", {
                    id: swap.id,
                    status: swap.process_status,
                });
                await this.storage.updateBridgeProcessStatus(swap.id, BridgeProcessStatus.ERROR_OPENING_WITHDRAW);
            ...
            if (
                withdraw_box.amount.toString() === swap.amount.toString() &&
    ...
    public async closeWithdraw(swap: IBridgeSwapInfoInternal) {
...

        {
            const task = async () => {
                try {
                    return {
                        withdraw_box: await ContractUtils.getTokenBridgeWithdrawLockBoxInfo(target_bridge, swap.id),
                    };
                } catch (error) {
                    const vm_error = ContractUtils.getVMError(error);
                    logger.error("Failed to check withdraw lock box opening", {
