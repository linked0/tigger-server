...
async function main() {
    ...
    */
    ...
    const bridge_biznet = new hre.ethers.Contract(
        process.env.BRIDGE_BIZNET_CONTRACT_ADDRESS || "",
        bridgeArtifact.abi,
        provider_biznet
    ) as BOACoinBridge;
    ...
    console.log(`contract address: ${process.env.BRIDGE_BIZNET_CONTRACT_ADDRESS || ""}`);
    const res = await bridge_biznet.connect(adminNonceManager).removeManager(manager.address);
    await res.wait();
}
