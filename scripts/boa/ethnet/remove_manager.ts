...
async function main() {
    ...
    */
    ...
    console.log(`contract address: ${process.env.BRIDGE_ETHNET_CONTRACT_ADDRESS || ""}`);
    const res = await bridge_ethnet.connect(adminNonceManager).removeManager(manager.address);
    await res.wait();
}
